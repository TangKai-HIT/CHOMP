%%CHOMP 2D point 2-order smoother: test CHOMP algorithm to plan a 2D point  
%%trajectory using 2-order differetial matrix with fixed boundary points
clc; clear; close all;
...
%% Init chomp planner
smooth_order = 2;
fullSmoothMatrix = 1; %specify if use full smooth matrix or not
chomp2D = chomp_wp_planner(p', smooth_order, true, fullSmoothMatrix);
...
[ A, K, b, c, e ] = get_smooth_matrices_part(chomp2D.num_pt , smooth_order, ...
                         start_pts, end_pts); % partitioned smooth matrices (not recommended, invalid boundary condition!)
end
...
%% Set options
chomp2D.options.Eta = 1*1e7; % for full smooth matrix case
