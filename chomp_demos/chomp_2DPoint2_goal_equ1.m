%%CHOMP 2D point 2-order smoother (goal equality)
% use CHOMP algorithm to plan a 2D point trajectory with goal equality constraint
clc; clear; close all;
...
%% Init chomp planner
smooth_order = 2;
chomp2D = chomp_wp_planner(p', smooth_order, false, true);

%% Define cost function
lambda1 = 1e-7;
lambda2 = 1*1e2;
[start_pts, ~] = chomp2D.get_boundary_pt();

[ A, K, b, c, e ] = get_smooth_matrices_goalpart(chomp2D.num_pt , smooth_order, ...
                             start_pts); % smooth matrices
...
%% Set options
chomp2D.options.Eta = 8*1e7; % for full smooth matrix case
% chomp2D.options.Eta = 5.5*1e7; % for part smooth matrix case 
...
chomp2D.options.FreeEndPoint = 1; %set end point free

%% Set constraint
goal_equ = {0.95, []}; %goal equality constraint
chomp2D.constraints = add_goal_equconstraints(chomp2D.constraints, goal_equ, chomp2D.num_pt);
%plot constraint
hold on; plot([goal_equ{1} goal_equ{1}], [-1e6 1e6], 'g--', 'LineWidth', 1.5);
...
figure("Position",[800,100, 800, 800]);
