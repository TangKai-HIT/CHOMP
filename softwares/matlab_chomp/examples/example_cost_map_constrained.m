...
%
%% EXAMPLE_COST_MAP_CONSTRAINED
% Create sample cost map and then do trajectory constrained CHOMP
clc;
clear;
close all;
...
%% Create a bound constraint on Y
lb = {[], 0.45};
ub = {[], 0.55};
constraints = get_bound_constraints( lb, ub, size(xi,1) - 2);
plot([p_start(1) p_end(1)], [lb{2} lb{2}], 'r--', 'LineWidth', 3);
plot([p_start(1) p_end(1)], [ub{2} ub{2}], 'r--', 'LineWidth', 3);
